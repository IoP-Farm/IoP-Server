...
Для работы всей системы можно также развернуть свой сервер и запустить соответствующие сервисы:
...
P.S: на данный момент все сервисы запущены в фоновом режиме на нашем сервере. Если у вас есть собственный сервер и собственный MQTT брокер - запустите MQTT на определенном порту и настройте в дефолтных настройках или в точке доступа WiFiManager новый хост и порт MQTT брокера для пересылки данных.
...
- В конфигурации устройства должен быть корректно задан `deviceId` в приложении, хост и порт сервера. Устройству изначально присвоен уникальный серийный номер, который вы должны зарегистрировать в приложении (будет сделано в дальнейшем). 
...
- Для web-интерфейса и ArduinoOTA требуется аутентификация.
