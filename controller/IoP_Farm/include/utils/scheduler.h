...
namespace farm::utils
{
    ...
    private:
        ...
        // Вектор запланированных событий
        ...
        // uint64_t - 8 байт, если событие создается каждую секунду, то тип переполнится через ~584 млрд лет
        ...
        // Проверка, совпадает ли текущее время с запланированным
        ...
        // ситуация: планировщик проходит по вектору событий, и в то же время генерируется событие
        // тогда вектор событий будет изменен, и планировщик будет проходить по нему некорректно
    ...
    public:
        ...
        // Проверка, выполнена ли первичная синхронизация с NTP
        bool isNtpOnline() const;
        
        // Инициализация планировщика (NTP, мьютексы)
        bool initialize(int8_t gmtOffset = farm::config::time::DEFAULT_GMT_OFFSET);
        
        // Проверка, инициализирован ли планировщик
        ...
        // Удалить запланированное событие по идентификатору
        ...
        // Получить количество запланированных событий
        ...
        // Установить интервал проверки расписания (в миллисекундах)
